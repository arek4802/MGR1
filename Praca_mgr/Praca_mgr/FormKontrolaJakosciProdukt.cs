...
namespace Praca_mgr
{
    public partial class FormKontrolaJakosciProdukt : Form
    {
        Firma_produkcyjnaEntities db;
        public FormKontrolaJakosciProdukt(Firma_produkcyjnaEntities db)
        {
            InitializeComponent();
            this.db = db;
        ...
        private void btnDodaj_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Kontrola_parametr_produkt kontrolaParametrProdukt = new Kontrola_parametr_produkt();
                int numRows = dgvvKontrolaProdukt.Rows.Count;
                kontrolaParametrProdukt.ID_kontrola_jakosci_produkt = int.Parse(this.dgvvKontrolaProdukt.Rows[numRows - 1].Cells[0].Value.ToString()) + 1;
                kontrolaParametrProdukt.ID_parametr_produkt = int.Parse(this.dgvvParametrProdukt.CurrentRow.Cells[0].Value.ToString());
                kontrolaParametrProdukt.Wartosc = decimal.Parse(txtWartosc.Text);
                db.Kontrola_parametr_produkt.Add(kontrolaParametrProdukt);
                db.SaveChanges();
            ...
            else
            {
                ...
                for (int i = startIndex; i < iloscProduktowKolejka; i++)
                {
                    ...
                    for (int j = 0; j <= iloscParametrow - 1; j++)
                    {
                        ...
                        int numRows = dgvvKontrolaProdukt.Rows.Count;
                        kontrolaParametrProdukt.ID_kontrola_jakosci_produkt = int.Parse(this.dgvvKontrolaProdukt.Rows[numRows - 1].Cells[0].Value.ToString()) + 1;
                        kontrolaParametrProdukt.ID_parametr_produkt = int.Parse(this.dgvvParametrProdukt.Rows[j].Cells[0].Value.ToString());
                        ...
                        kontrolaParametrProdukt.Wartosc = wartosc;
                        db.Kontrola_parametr_produkt.Add(kontrolaParametrProdukt);
                        db.SaveChanges();
                    }
                    refreshScreen();
        ...
        private void dgvvKontrolaJakosciKolejka_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            txtNazwa.Text = this.dgvvKontrolaJakosciKolejka.CurrentRow.Cells[2].Value.ToString();
        }

        private void dgvvParametrProdukt_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
